// src/lessonEditPage.js

async function loadLessonEditPage() {
  try {
    ...
    async function updateSubject(id, updatedData) {
      try {
        const response = await fetch(`http://localhost:3041/lessons/${id}`, {
    ...
    form.addEventListener("submit", async (event) => {
      ...
      const updatedSubjectUrl = `/lessons/?id=${id}`;
